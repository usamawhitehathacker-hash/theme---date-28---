...
4. Search usages before editing shared files such as `snippets/product-card.liquid`, `snippets/image.liquid`, `snippets/button.liquid`, `sections/_blocks.liquid`, `assets/product-form.js`, `assets/variant-picker.js`, and `assets/facets.js`.
5. Follow Shopify render order: `layout -> section group/template -> section -> block -> snippet -> asset -> config/settings -> locale`.
6. Keep CSS scoped to section/component selectors. Prefer `#shopify-section-{{ section.id }}` or existing Horizon section classes.
7. Use Horizon's ES module/custom element style. Prefer imports from import maps such as `@theme/component` and avoid global JS state.
8. Validate schema carefully: valid JSON, no trailing commas, range `(max - min) / step <= 100`, and defaults on the step grid.
9. Use translations for user-facing and customizer text when the theme pattern expects `t:` keys.
10. Preserve merchant settings and user changes. Do not overwrite `settings_data.json` casually.
11. Accessibility and performance are part of done: alt text, keyboard behavior, ARIA where needed, lazy images, responsive layout, no unnecessary blocking assets.
12. If implementing in the shared workspace, do the work end to end and verify. If the user is asking for guided learning or snippet-only help, present the plan and proceed step by step.
...
2. Inspect: read relevant source files and `references/horizon-source-map.md`; use `rg` for renders, asset URLs, block types, IDs, and translation keys.
3. Map: list create/modify/read/affected files and file dependencies.
4. Sequence: schema/settings first, then Liquid structure, snippets/blocks, assets, styles, templates/groups, locales, then verification.
5. Implement: follow existing Horizon patterns and keep edits scoped.
6. Verify: run available validation commands, syntax checks, searches for missing references, and browser/customizer checks when possible.
7. Report: summarize changed files, verification performed, and any residual risk.
